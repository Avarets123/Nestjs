...
export class UserController {
  ...
  @Get('addfriend/:name')
  @UseGuards(AuthGuard)
  async addFriend(@User('id') currentUserId: number, @Param('name') nameFriend: string): Promise<{}> {
    return await this.userService.addFriend(currentUserId, nameFriend);
  }

  @Get('delfriend/:name')
  @UseGuards(AuthGuard)
  async delFriend(@User('id') currentUserId: number, @Param('name') nameFriend: string): Promise<UserEntity> {
